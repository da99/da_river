...
River.prototype.job_must_find = function () {
  ...
  var f_new = function (j, last) {
    ...
    var old_fin = j.finish;
    j.finish = function (err_or_val, msg) {

      var val = err_or_val;
      if (arguments.length > 1 || (val && ((_.isArray(val)) ? val.length : true)))
        return old_fin.apply(j, arguments);

      if (group !== undefined && id !== undefined)
        var err = new Error(group + ', ' + id + ', not found.');
      else
        var err = new Error('At least one reply required. Value: ' + JSON.stringify(val));
      err.job = j;

      return old_fin.apply(j, ['not_found', err, j]);
    };
  ...
  args.push(f_new);
  return this.job.apply(this, args);
};
